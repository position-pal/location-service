...
import cats.implicits.{catsSyntaxEither, toFlatMapOps, toFunctorOps}
...
import lepus.protocol.domains.{ExchangeName, FieldTable, QueueName, ShortString}
...
class RabbitMQGroupsEventConsumer[F[_]: Async](userGroupsService: UserGroupsService[F]) extends RabbitMQ.Utils:
  ...
  def start(connection: Connection[F]): F[Unit] = connection.channel.use: ch =>
    for
      n <- Async[F].fromEither(QueueName.from(groupsEventsCollectorQueueName).leftMap(QueueNameCreationFailed(_)))
      q <- ch.queue.declare(queue = n, autoDelete = false, durable = true, exclusive = false)
      q <- Async[F].fromOption(q, QueueDeclarationFailed)
      e <- Async[F].fromEither(ExchangeName.from(groupsEventsExchangeName).leftMap(ExchangeNameGetFailed(_)))
      _ <- ch.queue.bind(q.queue, e, ShortString.empty)
      consumer = ch.messaging.consume[Array[Byte]](q.queue, mode = ConsumeMode.NackOnError)
  ...
  private def handleGroupEvent(event: Message[Array[Byte]]): F[Unit] = event.properties.headers match
    case Some(headers) if MessageType.MEMBER_ADDED.name() in headers =>
      userGroupsService.addedMember(serializer.deserializeAddedMemberToGroup(event.payload))
    case Some(headers) if MessageType.MEMBER_REMOVED.name() in headers =>
