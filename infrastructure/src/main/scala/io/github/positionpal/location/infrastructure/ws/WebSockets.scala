    ...
    def handleGroupRoute(groupId: String, userId: String, service: ActorBasedRealTimeTracking.Service[IO, UserId]): Flow[Message, Message, ?] =
      ...
      val routeToClient: Source[Message, ActorRef[Protocol]] =
        ActorSource.actorRef(
          ...
          println(s"Adding observer for $userId in group $groupId")
          ...
          activeSessions.getOrElse(groupId, Set.empty).foreach: (uid, _) =>
            println(s"Sending ref to $uid")
            service.addObserverFor(UserId(uid))(Set(ref)).unsafeRunSync()
          println(s"Ok to send ref to $userId")
          activeSessions(groupId) = activeSessions.getOrElse(groupId, Set.empty) + ((userId, ref))
          println(s"Active sessions: ${activeSessions.getOrElse(groupId, Set.empty)}")
