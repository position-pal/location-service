...
class MapboxServiceAdapterTest extends AnyFlatSpec with Matchers:
  ...
  "The Mapbox service adapter" should "calculate the distance between two locations" in:
    val distanceRequest = for
      ...
      distance <- mapboxServiceAdapter.distance(Driving)(cesenaCampusLocation, bolognaCampusLocation).value.run(config)
    yield distance
