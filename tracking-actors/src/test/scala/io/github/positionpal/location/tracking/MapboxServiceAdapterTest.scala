  ...
  describe("The Mapbox service adapter"):
    it("should calculate the distance between two locations"):
      val distance = (for
        ...
        dist <- mapService.distance(Driving)(cesenaCampus.location, bolognaCampus.location)
    ...
    it("should calculate the arrival time between two locations"):
      val arrivalTime = (for
        mapService <- mapServiceAdapter
        time <- mapService.duration(Driving)(cesenaCampus.location, bolognaCampus.location)
